...
**수업**: GPU의 시간을 좌우하는 것이 무엇인가, GPU가 왜 빠른가

`ECE 498AL, Lecture 2: The CUDA Programming MOdel` `CS231N, Lecture 6: ...Babysitting the Learning Process`

**Nerual Network GPU 처리과정**:

HDD에 저장된 이미지가 CPU를 통해 PCI Express bus에 꽂힌 GPU 카드로 넘어간다.
GPU카드는 GPU와 GDDR DRAM으로 구성되어 있다.
인공지능 SW와 HDD에 있는 이미지가 GDDR DRAM으로 전달되서 GPU가 이미지 처리를 한다.


**CPU에 비해 GPU의 속도가 20~30배 빠른 이유**:

CPU 만드는 회사로 인텔이 있고, GPU 만드는 회사로 NVIDIA가 있다.
요즘 CPU는 멀티코어이다.
보통 한 CPU당 코어가 4개씩 들어 있다.
이를 쿼드코어라고 한다.
반면에 좋은 GPU는 코어가 3500~5000개 들어 있다.
(우리가 쓰는 Dell Inspiron 15 7000 Gaming 노트북의 GPU에는 아마 수백 개 들어 있을 듯)

즉 CPU와 GPU는 코어가 약 1000배 차이가 난다.
그럼 왜 인텔은 코어 수를 늘리지 않았을까?
인텔 CPU의 목적은 Word, Excel, Powerpoint, 아래 한글, Internet Explorer, Chrome 사용자를 위한 것이다.
위 프로그램은 프로그램 자체의 병렬도가 굉장히 적다.
프로그램을 병렬처리하기 나쁘다는 뜻이다.
"A 다음에 B해. B 다음에 C해" 이런 식으로 처리하게 되면 A, B, C간에 dependency가 존재한다는 뜻이다.
즉 위 과정은 sequential(순차적)인 과정이다.
만약에 "A, B, C, D 등 눈치보지 말고 실행해."라고 한다면 병렬처리이다.
Neural Network의 matrix-vector multiplication은 병렬도가 굉장히 높다.
예를 들어, 1000 by 1000 = 백만 개의 연산을 해야 한다면 병렬도가 굉장히 높은 연산이다.

그리고 처음 나올 때부터 CPU는 sequential 프로그래밍을 염두에 둔 것이다.
가령, C언어에서는 다음과 같은 코드를 실행한다고 해보자.
...
c = a+b;
```
이때 a 다음에 b 다음에 c를 계산해야지 순서를 바꾸면 오류가 난다.
이를 sequential 과정이라고 한다.

인텔이 코어를 4개(쿼드)를 8개(옥타)로 늘리면 어떻게 될까?
우리가 엑셀을 사용하면 코어 하나가 엑셀을 담당한다.
그러니까 인텔의 목적은 코어 하나 하나가 성능이 좋아야 한다.
그래서 코어를 되게 복잡하게 만든다.
또 CPU에는 내부에 Cache Memory가 있다.
캐시 메모리란 내부에 SRAM이라는 빠른 메모리로 된 창고를 뜻한다.
왜 창고가 필요할까?
CPU에 있는 DRAM에 있는 프로그램을 갖고 와서 실행을 하는데, DRAM에 있는 데이터를 갖고 와서 처리하는데 100 cycle이 걸린다.
(앞 문장에 대해 정확한 설명 누락되었지만) 비유하면 6시간 통학해서 1시간 수업 듣는 셈이다.
그래서 CPU 내부에 있는 캐시 메모리를 이용한다.
같은 chip 안에 있는 메모리를 활용하면 전달 속도가 훨씬 빠르기 때문이다.
결국 물류의 문제이다.

Memory Wall이 있다.
옛날에는 CPU와 DRAM의 클럭 차이가 크지 않았다.
ex) APPLE 2 => CPU : 8mHz, DRAM : 100ns(=10mHz).
그런데 요즘엔 CPU가 3gHz가 되었는데 DRAM은 속도가 그대로이다.
DRAM의 용량은 무어의 법칙에 따라 증가했는데, 왜 성능(=속도)는 그대로일까?
DRAM 용량이 늘어났기 때문에 circuit 속도는 빨라졌지만 끄집어 내는 속도는 그대로이다.
왜냐하면 용량이 커지면 끄집어 내는 게 훨씬 더 복잡하기 때문이다.

인텔 CPU의 목적은 single core 속도를 희생하지 않는 것이다.
single core가 인텔 CPU를 사용하는 사람들이 이용하년 Word, Excel, Chrome 등의 프로그램을 각각 처리하기 때문이다.
게임과 같은 경우 그래픽 처리를 하고 그래픽 처리는 위로 그렸다가, 옆으로 그렸다가 병렬처리가 굉장히 중요해서 GPU가 필요하다.

코어 수가 많으면 기존에 프로그램을 짜는 사람한테 편리하지 않다.
기존에 프로그램을 짜는 과정은 sequential 과정이기 때문에 고성능 코어 소수 있는 게 낫다.
코어 4개라는 것은 고성능, 즉 똘똘한 하인을 4명으로 만드는 것이다.
하인을 그냥 많이 만들기보다는 똑똑한 하인을 소수로 만드는 게 CPU이다.

GPU chip에 있는 녹색, 노란색이 모두 코어 하나하나이다.
아주 기본 기능이 있는 코어가 여러 개이다.
GPU에는 캐시 메모리가 거의 없다. 아주 조금만 있다.
CPU에는 8~16MB인데 GPU에는 0.5MB 정도 된다.
캐시 메모리가 없으면 DRAM에서 가져오는데 시간이 많이 걸리는데 어떻게 해야 할까?

**GPU 컴퓨팅의 특징**:

캐시 메모리는 통학으로 치면 기숙사이다.
단, 공간이 제약되어 있으므로 기숙사를 많이 만들 수는 없다.
GPU는 Latency hiding scheduling을 한다.
Latency hiding scheduling이란 아이패드나 삼성의 S패드를 사서 기차나 버스 안에서 유튜브 강의를 듣는 것이다.
Latency hiding scheduling을 하려면 해야 할 일이 복수 개 있어야 한다.
해야 할 일이 10개쯤 있으면 기차 타고 가면서 하나씩 들으면 된다.
GPU가 3500~5000개 코어가 있으면 해야 하는 쓰레드(Thread) 숫자는 수만 개 이상이다.

`필기 놓침. 한 번 들으면 원리를 제대로 파악할 수 있으니 Recording 꼭 들어보자.`

즉 GPU 코어 수가 3500개라도 해야 할 일이 3500개 보다 훨씬 더 많아야 한다 (병렬도가 높은 job을 해야 한다).
Latency hiding scheduling이 그래야 힘을 발휘하기 때문이다.
...
코어 하나당 N번의 cycle을 돈다.
코어가 10개이면 N/10번 돌면 된다.

**DRAM acess 횟수**:

인공지능 프로그램은 DRAM access가 많다.
weight를 가져올 때마다 DRAM으로 전달하는데 이 과정이 매우 많기 때문이다.
GPU에서 training 할 때 batch size가 많을수록 빠르다.
만약 batch size가 100이 아니라 10이면 weight를 10배 더 많은 횟수로 가져와서 CNN을 10번 돌리는 것이다.
batch size가 10으로 돌리는 것과 100으로 돌리는 것을 비교하면, batch size가 10일 때 DRAM access를 10배 더 많이 해야 한다.
training 할 때는 GPU가 유리하지만 inference할 때는 batch size를 늘리지 못하므로 GPU가 유리하지 않다.

**GPU 프로그래밍 방법**:

CPU는 functional-level parallel이다.
첫 번째는 크롬 틀어라, 두 번째는 유튜브 틀어라, 세 번째는 Gmail 틀어라, .. 등등.
즉 해야 하는 function이 모두 다르다.

GPU는 Data-level parallel이다.
병렬도가 최소 10,000개 이다.
사단에 병사가 10,000명이 있다.
사단장이 뒤로 돌아, 하면 병사 만 명이 모두 뒤로 돈다.
즉 처리하는 명령은 하나이다.
명령 1개, 데이터는 10,000개, 계산 결과는 10,000개이다.
GPU 프로그래밍에서는 프로그램을 한 개 짜면 10,000개가 작동한다.

**CUDA**:

NVIDIA가 만든 라이브러리이다.
간단히 얘기하면 data-level parallel과 같은 개념이다.

TensorFlow나 PyTorch는 CUDA Library로 바뀌고 CUDA Library는 GPU를 실행시킨다.
속도를 높이기 위해서 코드를 더 잘 짜는 것은 소용없다.
속도 면에서는 DRAM access가 중요하므로 batch size를 바꾸거나, 한꺼번에 여러 개를 inference하면 속도 개선을 할 수 있다.
